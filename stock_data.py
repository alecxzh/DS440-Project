...
def add_bayesian_indicators(df):
    ...
    for ticker in df['Ticker'].unique():
        ...
        # Assign in the same order as ticker_data (sorted by Date)
        df.loc[df[ticker_mask].sort_values("Date").index, 'Posterior_Trend'] = posterior
        df.loc[df[ticker_mask].sort_values("Date").index, 'Prior_Up'] = prior_up
        df.loc[df[ticker_mask].sort_values("Date").index, 'Posterior_Up'] = posterior_up
        df.loc[df[ticker_mask].sort_values("Date").index, 'Is_Large_Move'] = large_move
